...
import {
    ...
    fromText,
    PolicyId, UTxO, Unit, 
    Lucid, Blockfrost, TxComplete, TxHash 
} from "lucid-cardano";
...
const MintNFT = () => {
    ...
    const { lucid, walletAddress } = appState
    const [serializedNFT, setSerializedNFT] = useState('')
    ...
    const getFinalPolicy = async (utxo: UTxO): Promise<GetFinalPolicy> => {     // apply params to the serialized minting policy
        const tn = fromText("Simple Oracle's NFT");    // Get the HEX value of the string
    ...
    const mintNFT = async () => {
        ...
        const wAddr: any = await walletAddress
        ...
        <section className="py-8 px-4 m-6">
            ...
            <form>
                ...
                <div>
                    ...
                    <input type="text" name="" className="mt-5 p-2.5 w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-gray-300 dark:border-gray-600 dark:placeholder-gray-700 dark:text-gray dark:focus:ring-blue-200 dark:focus:border-blue-200" placeholder="Developer Address ..." required />
                </div>
